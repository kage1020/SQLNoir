...
export function CaseFile({ caseData, onClick, isSolved }: CaseFileProps) {
  const { t } = useTranslation(["cases", "dashboard"]);
    ...
    >
      {caseData.isNew && !isSolved && (
        <div className="absolute top-0 -right-2 z-20">
          <div className="relative">
            ...
            >
              <span className="text-sm tracking-wider">{t("dashboard:new")}</span>
            </div>
          </div>
        </div>
      )}
      {isSolved && (
        <div className="absolute inset-0 flex items-center justify-center z-10">
          ...
          >
            <div className="text-2xl tracking-[0.2em] font-bold">{t("dashboard:solved")}</div>
          </div>
        </div>
      )}
      ...
      <p className="text-amber-800/80 text-sm mb-4">{t(`cases:${caseKey}.description`)}</p>
      <div className="flex justify-between items-center">
        <span className="text-xs font-mono bg-amber-100/80 backdrop-blur-sm px-2 py-1 rounded">
          {t("dashboard:xp", { count: caseData.xpReward })}
        </span>
      </div>
    </div>
  );
